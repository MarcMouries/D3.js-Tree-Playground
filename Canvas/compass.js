...
function Compass({ center, radius, bearing_point }) {
  ...
  Compass.prototype.setBearingPoint = function (bearing_point) {
    if (!bearing_point) {
      console.log("none: bearing_point");
      this.startDegrees = 0;
      this.endDegrees = 360;
    } else
    {
  ...
  Compass.prototype.findBearingAngle = function (bearing_point) {
    ...
    if (angle < 0) var bearingTT = (360 + angle) % 360;else
    var bearingTT = angle;
  
    console.log("findBearingAngle: " + bearingTT);
    return bearingTT;
  };
  ...
  Compass.prototype.calculateOrientation = function () {
    ...
    for (var i = 0; i < number_of_points; i++) {
      ...
      log(this.bearing + ": " + start_angle + " - " + end_angle + msg_mid_point);
